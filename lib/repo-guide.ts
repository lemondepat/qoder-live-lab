...
const topics = [
  ...
  { match: /file|scope|edit|change|修改|文件|范围/i, answer: "Audience requests may change only the showcase source, its tests, and static assets. The market data fixture, control plane, runner, contracts, Ops, CI, dependency manifests, and security configuration are protected independently of the agent.", files: ["AGENTS.md", "packages/contracts/src/policy.ts"] },
  ...
  { match: /market|price|stock|finance|行情|股价|金融/i, answer: "The financial dashboard has a trusted read-only data plane and an agent-editable presentation plane. Qoder may change layout, charts, filters, and animation, but it cannot rewrite market facts, add trading actions, or remove the display-only disclaimer.", files: ["apps/showcase/src/market-data.ts", "packages/contracts/src/policy.ts"], ruleId: "DATA-001" },
