...
import { createBoundaryChallenge, finishRequest, getBoard, resetOpeningRelease, rollbackRelease, setSystem } from "@/lib/store";
import { GUARDRAIL_CHALLENGES } from "@qoder-live-lab/contracts/policy";

const schema = z.object({ action: z.enum(["pause", "resume", "provider-qca", "provider-local", "rollback", "opening-baseline", "cancel", "challenge"]), challenge: z.string().optional() });
