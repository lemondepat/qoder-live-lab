...
export default function Home() {
  ...
  return (
    <main className="shell">
      ...
      <section className="hero" id="top"><div className="hero-copy"><p className="kicker"><span>01</span> Build in public</p><h1>Shape the market.<br /><em>Watch it ship.</em></h1><p className="lede">Ask for a better way to see Hong Kong&apos;s trading day. Qoder builds the feature, tests its boundaries, and publishes a verified market experience.</p></div><div className="signal-card" aria-label="Current agent activity"><div className="signal-head"><span>Agent signal</span><span className="signal-dot" /></div><div className="orb"><i /><b>Q</b></div><div className="signal-meta"><strong>{activeRequest ? `Building ${activeRequest.id}` : "Ready for the next idea"}</strong><span>{activeRequest?.events.at(-1)?.message ?? "Guardrails online"}</span></div></div></section>
      <section className="request-panel" aria-labelledby="request-heading"><div><p className="section-index">02 / YOUR TURN</p><h2 id="request-heading">What should the market dashboard do next?</h2><div className="boundary-prompts"><span>TEST THE GUARDRAILS</span>{GUARDRAIL_CHALLENGES.slice(0, 3).map((challenge) => <button key={challenge} onClick={() => { setRequest(challenge); setNotice("This challenge should be blocked safely."); }}>{challenge}</button>)}</div></div><div className="request-form"><label>YOUR NICKNAME<input value={author} onChange={(event) => setAuthor(event.target.value)} maxLength={24} /></label><textarea value={request} onChange={(event) => setRequest(event.target.value)} placeholder="Try: Turn the plain stock list into a sector heatmap…" maxLength={200} aria-label="Describe your feature request" /><div className="form-foot"><span>{request.length}/200</span><button disabled={submitting || board.system.queuePaused} onClick={() => submitRequest()}>{board.system.queuePaused ? "Queue paused" : submitting ? "Submitting…" : "Join the queue"} <b>↗</b></button></div>{notice && <p className="notice" role="status">{notice}</p>}</div></section>
      ...
      <section className="guardrails" id="guardrails"><div><p className="section-index">04 / GUARDRAILS</p><h2>Autonomous.<br />Not unbounded.</h2></div><div className="guardrail-list">{[["SCOPE-001", "The agent may edit the market canvas — never the control plane."],["TESTS-001", "Tests can be added and strengthened, never removed or skipped."],["SECRETS-001", "Production credentials are never available inside the agent sandbox."],["GIT-001", "Only the trusted runner can merge and activate a release."]].map(([id, copy]) => <div className="guardrail" key={id}><span>{id}</span><p>{copy}</p><b>ENFORCED</b></div>)}</div></section>
