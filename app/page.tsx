...
export default function Home() {
  ...
  const [blockedEvidenceOpen, setBlockedEvidenceOpen] = useState(false);
  ...
  return (
    <main className="shell">
      <header className="topbar"><a className="brand" href="#top" aria-label="Qoder Live Lab home"><span className="brand-mark">Q</span><span>Qoder Live Lab</span></a><nav className="top-links"><a href="/stage">Stage</a><a href="/releases">Releases</a><a href="/ops">Ops</a><div className="live-pill"><span />LIVE · {board.system.activeRelease.version}</div></nav></header>
      <section className="hero" id="top"><div className="hero-copy"><p className="kicker"><span>01</span> Build in public</p><h1>Shape the market.<br /><em>Watch it ship.</em></h1><p className="lede">Redesign an entire live Hong Kong market experience—not just a widget. Qoder transforms the screen, verifies the boundary, and publishes it while trusted market facts keep moving underneath.</p></div><div className="signal-card" aria-label="Current agent activity"><div className="signal-head"><span>Agent signal</span><span className="signal-dot" /></div><div className="orb"><i /><b>Q</b></div><div className="signal-meta"><strong>{activeRequest ? `Building ${activeRequest.id}` : "Ready for the next idea"}</strong><span>{activeRequest?.events.at(-1)?.message ?? "Guardrails online"}</span></div></div></section>
      <section className="request-panel" aria-labelledby="request-heading"><div><p className="section-index">02 / YOUR TURN</p><h2 id="request-heading">How would you transform the live market?</h2><div className="idea-prompts"><span>GO BIG · TRY ONE</span>{REHEARSAL_FEATURES.slice(0, 3).map((feature) => <button key={feature.id} onClick={() => { setRequest(feature.request); setNotice("Big transformation ready — edit it or send it."); }}><b>{feature.title}</b><small>{feature.impact}</small></button>)}</div><div className="boundary-prompts"><span>OR TEST THE GUARDRAILS</span>{GUARDRAIL_CHALLENGES.slice(0, 3).map((challenge) => <button key={challenge} onClick={() => { setRequest(challenge); setNotice("This challenge should be blocked safely."); }}>{challenge}</button>)}</div></div><div className="request-form"><label>YOUR NICKNAME<input value={author} onChange={(event) => setAuthor(event.target.value)} maxLength={24} /></label><textarea value={request} onChange={(event) => setRequest(event.target.value)} placeholder="Describe a full-screen heatmap, broadcast wall, volatility environment…" maxLength={200} aria-label="Describe your feature request" /><div className="form-foot"><span>{request.length}/200</span><button disabled={submitting || board.system.queuePaused} onClick={() => submitRequest()}>{board.system.queuePaused ? "Queue paused" : submitting ? "Submitting…" : "Join the queue"} <b>↗</b></button></div>{notice && <p className="notice" role="status">{notice}</p>}</div></section>
      <section className="board" aria-labelledby="board-heading"><div className="board-title"><div><p className="section-index">03 / LIVE PIPELINE</p><h2 id="board-heading">Every change leaves evidence.</h2></div><span>{board.system.provider.toUpperCase()} · Auto-refresh · 2s</span></div><div className="lanes">{lanes.map((lane) => <section className="lane" key={lane.key}><header><div><span className={`lane-dot ${lane.key}`} />{lane.label}</div><small>{lane.eyebrow}</small></header><div className="lane-cards">{(grouped[lane.key] ?? []).map((card: ChangeRequest) => <RequestCard key={card.id} card={card} />)}{(grouped[lane.key] ?? []).length === 0 && <div className="empty-card">{lane.key === "queued" ? "Your idea could be next." : "No changes here right now."}</div>}</div></section>)}</div>{latestBlocked && <><div className="blocked-strip"><div className="blocked-icon">×</div><div><span>CHANGE BLOCKED · {latestBlocked.policy?.ruleId ?? "POLICY"}</span><strong>{latestBlocked.policy?.publicReason ?? "Candidate did not pass the guardrails."}</strong><p>0 files promoted · Live version remains {board.system.activeRelease.version}</p></div><button aria-expanded={blockedEvidenceOpen} aria-controls="latest-blocked-evidence" onClick={() => setBlockedEvidenceOpen((open) => !open)}>{blockedEvidenceOpen ? "Hide evidence" : "View evidence"}</button></div>{blockedEvidenceOpen && <BlockedEvidence card={latestBlocked} liveVersion={board.system.activeRelease.version} />}</>}</section>
      <section className="guardrails" id="guardrails"><div><p className="section-index">04 / GUARDRAILS</p><h2>Autonomous.<br />Not unbounded.</h2></div><div className="guardrail-list">{[["SCOPE-001", "The agent may rebuild the entire market experience — never the control plane."],["DATA-001", "Live market facts and feed health come only from the trusted data plane."],["SECRETS-001", "Production and market-provider credentials never enter the agent sandbox."],["GIT-001", "Only the trusted runner can merge and activate a release."]].map(([id, copy]) => <div className="guardrail" key={id}><span>{id}</span><p>{copy}</p><b>ENFORCED</b></div>)}</div></section>
...
function BlockedEvidence({ card, liveVersion }: { card: ChangeRequest; liveVersion: string }) {
  return <section className="blocked-evidence" id="latest-blocked-evidence" aria-label={`Blocking evidence for ${card.id}`}><header><div><span>BOUNDARY EVIDENCE</span><strong>{card.id}</strong></div><b>{card.status.toUpperCase()}</b></header><div className="blocked-evidence-grid"><div><small>REQUIREMENT / REQUESTER</small><strong>{card.title}</strong><p>{card.author}</p></div><div><small>QCA SESSION / LAYER</small><strong>{card.qcaSessionId ?? "Stopped before session creation"}</strong><p>{card.policy?.layer ?? "input"} enforcement</p></div><div><small>PROMOTION RESULT</small><strong>0 files promoted</strong><p>Live version unchanged · {liveVersion}</p></div></div><div className="blocked-policy"><small>POLICY DECISION</small><b>{card.policy?.ruleId ?? "POLICY"}</b><strong>{card.policy?.publicReason ?? "Candidate did not pass the guardrails."}</strong>{card.policy?.evidence.map((item) => <span key={item}>↳ {item}</span>)}</div><footer><span>FILES CHANGED · {card.files?.length ?? 0}</span><span>COMMIT · NONE</span><span>PR · NONE</span><span>PREVIEW · NONE</span></footer></section>;
}
