  ...
  return (
    <main className="stage-shell">
      <header className="stage-bar">
        ...
        <div className="stage-now"><span>{activeRequest ? "BUILDING NOW" : "CURRENT RELEASE"}</span><strong>{activeRequest?.title ?? release?.requirement ?? "Creative canvas"}</strong></div>
      ...
      <div className="stage-frame-wrap" key={release?.previewUrl}>
        <iframe title="Current verified creative canvas" src={release?.previewUrl || "/showcase"} sandbox="allow-scripts allow-pointer-lock" />
      </div>
