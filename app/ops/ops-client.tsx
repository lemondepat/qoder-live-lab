...
import type { BoardSnapshot } from "@qoder-live-lab/contracts";
...
export function OpsClient() {
  ...
  return (
    <main className="ops-shell">
      ...
      <section className="ops-hero"><div><p>TRUSTED CONTROL PLANE</p><h1>Run fast.<br /><em>Stay bounded.</em></h1></div><div className="ops-status"><span>RUNNER</span><b className={board?.system.runnerLastSeenAt ? "ok" : ""}>{board?.system.runnerLastSeenAt ? "CONNECTED" : "WAITING"}</b><small>{board?.system.provider.toUpperCase() ?? "QCA"} provider · {board?.system.queuePaused ? "Queue paused" : "Auto mode"}</small></div></section>
