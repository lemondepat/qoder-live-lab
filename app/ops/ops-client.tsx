  ...
  return (
    <main className="ops-shell">
      ...
      <section className="ops-grid">
        ...
        <article className="ops-card wide"><span className="ops-label">FEATURE LAUNCHPAD</span><h2>Launch a pre-verified transformation.</h2><p>Each signed pack is already built on the trusted data contract. The controller creates an isolated branch and still requires policy, tests, GitHub checks, Preview health, and merge protection before it can go live.</p><div className="feature-grid">{REHEARSAL_FEATURES.map((feature, index) => <button key={feature.id} disabled={!authenticated} onClick={() => act("feature", undefined, feature.id)} style={{ "--feature-accent": feature.accent } as React.CSSProperties}><span>0{index + 1} · SIGNED FEATURE PACK</span><strong>{feature.title}</strong><small>{feature.impact}</small><i>READY · QUEUE ↗</i></button>)}</div></article>
