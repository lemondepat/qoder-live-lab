...
import { OPENING_RELEASE_VERSION, type BoardSnapshot, type MarketSnapshot } from "@qoder-live-lab/contracts";
...
export function OpsClient() {
  ...
  async function act(action: string, challenge?: string) {
    const response = await fetch("/api/ops/action", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ action, challenge }) });
  ...
  return (
    <main className="ops-shell">
      ...
      <section className="ops-grid">
        ...
        <article className="ops-card wide"><span className="ops-label">DORMANT FEATURE LIBRARY</span><h2>Ready for Qoder to discover.</h2><p>These accelerators are source files only. They cannot change Stage directly; Qoder may reuse and adapt one only after an audience ticket semantically matches it, and the resulting candidate still passes the complete release pipeline.</p><div className="feature-grid">{REHEARSAL_FEATURES.map((feature, index) => <div className="feature-library-card" key={feature.id} style={{ "--feature-accent": feature.accent } as React.CSSProperties}><span>0{index + 1} · DORMANT ACCELERATOR</span><strong>{feature.title}</strong><small>{feature.impact}</small><i>QCA DISCOVERS · NOT CONNECTED</i></div>)}</div></article>
